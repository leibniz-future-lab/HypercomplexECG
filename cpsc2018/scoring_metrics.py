...
def evaluate_12ECG_score(
    ...
    print("- UAR...")
    uar = compute_uar(_truth, _binary_pred)
...
def compute_uar(labels: np.ndarray, outputs: np.ndarray) -> float:
    num_recordings, num_classes = np.shape(labels)
    A = compute_confusion_matrices(labels, outputs)
    uar = np.zeros(num_classes)
    for k in range(num_classes):
        tp, fp, fn, tn = A[k, 1, 1], A[k, 1, 0], A[k, 0, 1], A[k, 0, 0]
        if tp + fn:
            uar[k] = float(tp) / float(tp + fn)
        else:
            uar[k] = float("nan")

    macro_uar = np.nanmean(uar)
