...
from .cinc2020_database import CINC2020 as CR
...
class CPSC2018(ReprMixin, Dataset):
    """ """

    __DEBUG__ = False
    __name__ = "CPSC2018"

    def __init__(self, config: CFG, training: bool = True, lazy: bool = True, test: bool = False) -> None:
        """
        Parameters
        ----------
        config: dict,
        ...
        training: bool, default True,
            if True, the training set will be loaded, otherwise the test/val set
        lazy: bool, default True,
            if True, the data will not be loaded immediately
        lazy: bool, default False,
            if True, the test data will be loaded instead of validation set
        ...
        if not self.lazy:
            self._load_all_data()
            print(f"Number of signals: {len(self._signals)}")
            print(f"Number of labels: {len(self._labels)}")
    ...
    def _train_test_split(
        ...
        _test_ratio = (100 - _train_ratio) // 2
        _val_ratio = 100 - _train_ratio - _test_ratio
        ...
        val_file = self.reader.db_dir_base / f"validation_ratio_{_test_ratio}{file_suffix}"

        if force_recompute or not all([train_file.is_file(), test_file.is_file(), val_file.is_file()]):
            tranche_records = {t: [] for t in _TRANCHES}
            train_set = {t: [] for t in _TRANCHES}
            test_set = {t: [] for t in _TRANCHES}
            val_set = {t: [] for t in _TRANCHES}
            for t in _TRANCHES:
                print(f"tranch: {t}")
                with tqdm(
            ...
            for t in _TRANCHES:
                is_valid = False
                if len(tranche_records[t]) > 0:
                    while not is_valid:
                        shuffle(tranche_records[t])
                        split_idx_train = int(len(tranche_records[t]) * train_ratio)
                        train_set[t] = tranche_records[t][:split_idx_train]
                        test_val_set = tranche_records[t][split_idx_train:]

                        split_idx_val = int(len(test_val_set) * 0.5)
                        test_set[t] = test_val_set[:split_idx_val]
                        val_set[t] = test_val_set[split_idx_val:]
                        print(f"Data split: {len(train_set[t])} in train\n"
                              f"{len(val_set[t])} in validation\n"
                              f"{len(test_set[t])} in test")
                        is_valid = self._check_train_test_split_validity(
                            train_set[t], test_set[t], val_set[t], set(TrainCfg.tranche_classes[t])
                        )
            train_file.write_text(json.dumps(train_set, ensure_ascii=False))
            test_file.write_text(json.dumps(test_set, ensure_ascii=False))
            val_file.write_text(json.dumps(val_set, ensure_ascii=False))
        else:
            train_set = json.loads(train_file.read_text())
            test_set = json.loads(test_file.read_text())
            val_set = json.loads(val_file.read_text())

        _tranches = list(self.tranches or "A")
        if self.training:
            records = list_sum([train_set[k] for k in _tranches])
        elif self.test:
            records = list_sum([test_set[k] for k in _tranches])
        else:
            records = list_sum([val_set[k] for k in _tranches])
        return records

    def _check_train_test_split_validity(
        self, train_set: List[str], test_set: List[str], val_set: List[str], all_classes: Set[str]
    ) -> bool:
        """
        ...
        val_set: list of str,
            list of the records in the validation set
        ...
        train_classes = set(
            reduce(add, [self.reader.get_labels(rec, fmt="a", normalize=False) for rec in train_set])
        )
        train_classes.intersection_update(all_classes)
        test_classes = set(
            reduce(add, [self.reader.get_labels(rec, fmt="a", normalize=False) for rec in test_set])
        )
        test_classes.intersection_update(all_classes)
        val_classes = set(
            reduce(add, [self.reader.get_labels(rec, fmt="a", normalize=False) for rec in val_set])
        )
        val_classes.intersection_update(all_classes)
        is_valid = len(all_classes) == len(train_classes) == len(test_classes) == len(val_classes)
        print(
