...
from torch_ecg.utils.download import _stem, http_get
...
from torch_ecg.databases.base import DEFAULT_FIG_SIZE_PER_SEC, PhysioNetDataBase, DataBaseInfo, _PlotCfg
...
class CINC2020(PhysioNetDataBase):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.db_tranches = list("A")
        # self.db_tranches = list("ABCDE")
    ...
    def _ls_rec(self) -> NoReturn:
        ...
        if record_list_fp.is_file():
            ...
            for tranche in self.db_tranches:
                # t = self._all_records[tranche]
                # print(f"records: {self._all_records.keys()}")
                # print(f"records: {len(self._all_records)}")
                # print(f"records: {t}")
                # print(f"tranche: {tranche}")
                self.db_dirs[tranche] = (
                    # self.db_dir_base / Path(t[0]).parent
