...
class CPSC2021(ReprMixin, Dataset):
    ...
    def __init__(
        self, config: CFG, task: str, training: bool = True, lazy: bool = True, test: bool = False
    ) -> None:
        """
    ...
    def __set_task(self, task: str, lazy: bool = True) -> None:
        ...
        if self.training:
            ...
            self.subjects = split_res.validation
        ...
        if self.task in [
            ...
            with tqdm(
                ...
                for idx in pbar:
                    d, l, m = self.fdr[idx]
                    nr_chs = d.shape[0]
                    if nr_chs < self.config.n_leads:
                        d = np.pad(d, [(0, self.config.n_leads - nr_chs), (0, 0)], mode='constant', constant_values=0)
    ...
    def _train_test_split(
        ...
        _val_ratio = int(_test_ratio / (_train_ratio + _test_ratio) * 100)
        assert _train_ratio * _test_ratio > 0

        train_file = self.reader.db_dir_base / f"train_ratio_{_train_ratio}.json"
        val_file = self.reader.db_dir_base / f"validation_ratio_{_test_ratio}.json"
        test_file = self.reader.db_dir_base / f"test_ratio_{_test_ratio}.json"

        if force_recompute or not all([train_file.is_file(), val_file.is_file(), test_file.is_file()]):
            ...
            afp_test = DEFAULTS.RNG_sample(
                    list(afp_subjects), int(round(len(afp_subjects) * _test_ratio / 100))
                ).tolist()
            aff_test = DEFAULTS.RNG_sample(
                    list(aff_subjects), int(round(len(aff_subjects) * _test_ratio / 100))
                ).tolist()
            normal_test = DEFAULTS.RNG_sample(
                    list(normal_subjects), int(round(len(normal_subjects) * _test_ratio / 100))
                ).tolist()

            test_set = (afp_test + aff_test + normal_test)

            afp_subjects = afp_subjects - set(afp_test)
            aff_subjects = aff_subjects - set(aff_test)
            normal_subjects = normal_subjects - set(normal_test)

            val_set = ( ## test ratio
                DEFAULTS.RNG_sample(
                    list(afp_subjects), int(round(len(afp_subjects) * _val_ratio / 100))
                ).tolist()
                + DEFAULTS.RNG_sample(
                    list(aff_subjects), int(round(len(aff_subjects) * _val_ratio / 100))
                ).tolist()
                + DEFAULTS.RNG_sample(
                    list(normal_subjects),
                    int(round(len(normal_subjects) * _val_ratio / 100)),
                ).tolist()
            )
            train_set = list(all_subjects - set(test_set) - set(val_set))

            DEFAULTS.RNG.shuffle(test_set)
            DEFAULTS.RNG.shuffle(train_set)
            DEFAULTS.RNG.shuffle(val_set)

            print(f"Data split: {len(train_set)} in train, {len(val_set)} in validation, {len(test_set)} in test")

            train_file.write_text(json.dumps(train_set, ensure_ascii=False))
            val_file.write_text(json.dumps(test_set, ensure_ascii=False))
            test_file.write_text(json.dumps(test_set, ensure_ascii=False))
            print(
                nildent(
                    f"""
                train set saved to \n\042{str(train_file)}\042
                validation set saved to \n\042{str(val_file)}\042
