"""
Adapted from: https://github.com/DeepPSP/torch_ecg/blob/master/torch_ecg/models/cnn/resnet.py
...
from torch_ecg.utils.misc import dict_to_str, add_docstring
...
__all__ = [
    "ResNet_PHC",
    "ResNetBasicBlock_PHC",
    "ResNetBottleNeck_PHC",
]


class ResNetBasicBlock_PHC(nn.Module, SizeMixin):
    """
    ...
    def __init__(
        ...
        **config,
        ...
        #             print(f"Module {module2} number of parameters: {sum(p.numel() for p in module2.parameters())}")
        #     else:
        #         print(f"Module {module} number of parameters: {sum(p.numel() for p in module.parameters())}")
...
class ResNetBottleNeck_PHC(nn.Module, SizeMixin):
    """
...
class ResNetStem_PHC(nn.Sequential, SizeMixin):
    """
    the input stem of ResNet
    """

    __name__ = "ResNetStem_PHC"
...
class ResNet_PHC(nn.Sequential, SizeMixin):
    """
    ...
    __DEBUG__ = False
    __name__ = "ResNet_PHC"
    building_block = ResNetBasicBlock_PHC
    ...
    def __init__(self, n: int, in_channels: int, **config) -> NoReturn:
        ...
        if not isinstance(self.config.get("building_block", None), str):
            ...
            for b in self.config.building_block:
                if b.lower() in [
                    ...
                    self.building_block.append(ResNetBottleNeck_PHC)
                    ...
                    self.building_block.append(ResNetBasicBlock_PHC)
            ...
            self.building_block = ResNetBottleNeck_PHC
        ...
        for macro_idx, nb in enumerate(self.config.num_blocks):
            ...
            # print(f"Dropouts: {self.__dropouts}")
            for block_idx in range(nb):
                # print(self.__dropouts[macro_idx])
                self.add_module(
